...
def load_data(opt):
    ...
    if opt.dataset in ['cifar10']:
        ...
        classes = {
            'tshirt': 0, 'trouser': 1, 'pullover': 2, 'dress': 3, 'coat': 4,
            'sandal': 5, 'shirt': 6, 'sneacker': 7, 'bag': 8, 'boot': 9
        }
...
##
def get_cifar_anomaly_dataset(trn_img, trn_lbl, tst_img, tst_lbl, abn_cls_idx=0, manualseed=-1, perc_outlier=None):
    """[summary]
    ...
    idx = np.arange(len(abn_tst_img))
    np.random.seed(manualseed)
    np.random.shuffle(idx)
    abn_tst_len = int(len(idx) * 1)
    abn_tst_idx = idx[:abn_tst_len]
    abn_tst_img = abn_tst_img[abn_tst_idx]
    abn_tst_lbl = abn_tst_lbl[abn_tst_idx]
    ...
    if (perc_outlier==None):
        ...
        if (perc_outlier>0.3):
            nrm_tst_len = int((1-perc_outlier)/perc_outlier * np.concatenate((abn_trn_img, abn_tst_img), axis=0).shape[0])
            new_tst_img = np.concatenate((nrm_tst_img[:nrm_tst_len], abn_trn_img, abn_tst_img), axis=0)
            new_tst_lbl = np.concatenate((nrm_tst_lbl[:nrm_tst_len], abn_trn_lbl, abn_tst_lbl), axis=0)
        else:
            abn_tst_len = int(perc_outlier/(1-perc_outlier) * nrm_tst_img.shape[0])
            new_tst_img = np.concatenate((nrm_tst_img, np.concatenate((abn_trn_img, abn_tst_img), axis=0)[:abn_tst_len]), axis=0)
            new_tst_lbl = np.concatenate((nrm_tst_lbl, np.concatenate((abn_trn_lbl, abn_tst_lbl), axis=0)[:abn_tst_len]), axis=0)
            

    return new_trn_img, new_trn_lbl, new_tst_img, new_tst_lbl

##
def get_mnist_anomaly_dataset(trn_img, trn_lbl, tst_img, tst_lbl, abn_cls_idx=0, manualseed=-1, perc_outlier=None):
    """[summary]
    ...
    if (perc_outlier==None):
        ...
        if (perc_outlier>0.3):
            nrm_tst_len = int((1-perc_outlier)/perc_outlier * torch.cat((abn_trn_img, abn_tst_img), axis=0).shape[0])
            new_tst_img = torch.cat((nrm_tst_img[:nrm_tst_len], abn_trn_img, abn_tst_img), axis=0)
            new_tst_lbl = torch.cat((nrm_tst_lbl[:nrm_tst_len], abn_trn_lbl, abn_tst_lbl), axis=0)

        else:
            abn_tst_len = int(perc_outlier/(1-perc_outlier) * nrm_tst_img.shape[0])
            new_tst_img = torch.cat((nrm_tst_img, torch.cat((abn_trn_img, abn_tst_img), axis=0)[:abn_tst_len]), axis=0)
            new_tst_lbl = torch.cat((nrm_tst_lbl, torch.cat((abn_trn_lbl, abn_tst_lbl), axis=0)[:abn_tst_len]), axis=0)
...
def get_fashion_mnist_anomaly_dataset(trn_img, trn_lbl, tst_img, tst_lbl, abn_cls_idx=0, manualseed=-1):
    """[summary]
